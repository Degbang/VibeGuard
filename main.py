...
modules implemented so far (cwe_798.py, cwe_284.py, cwe_287.py,
cwe_20.py) - this command parses .java/.properties/.yml/.yaml files
AND runs those rules against every successfully-parsed file. It is
not yet the full
...
def _run_rules(result: ScanResult) -> tuple[Finding, ...]:
    ...
    Only rules/cwe_798.py, rules/cwe_284.py, rules/cwe_287.py, and
    rules/cwe_20.py exist so far; more CWE rule modules get added here
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "CWE-20 improper input validation) against them. Findings are "
            "unscored rule matches, not graded risk - Layers 2-5 (feature "
            "extraction, rule-based scoring, ML classification, SHAP "
            "explainability) are not implemented yet, and CWE-1035 (known-"
            "vulnerable components) doesn't exist yet either."
        ),
    )
    parser.add_argument(
        "path",
